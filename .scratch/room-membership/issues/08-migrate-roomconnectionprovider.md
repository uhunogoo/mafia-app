...
- [x] The `connect` callback signature (`client.joinById(roomId, { token, name, password })`) and the `ready = Boolean(token && identity?.password)` check are preserved.
