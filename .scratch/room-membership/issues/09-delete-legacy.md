...
- [x] `src/lib/identity.ts` is deleted.
- [x] `src/components/Providers/PageProvider/` (the folder and both files: `PageProvider.tsx`, `index.ts`) is deleted.
- [x] `src/app/room/[roomId]/layout.tsx` no longer mounts `PageProvider`; only `RoomMembershipProvider` remains.
- [x] Repo-wide search for `@/lib/identity` returns no source-tree matches (only the spec and this issue's blocking references are allowed).
- [x] Repo-wide search for `PageContext` and `PageProvider` returns no source-tree matches.
